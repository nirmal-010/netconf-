...
export const buildAristaEOS = (state) => {
  ...
  let cli = `! Arista EOS Native Enterprise Configuration\n`;
  cli += `! Target Platform: Arista 7050X/7280R Data Center Core Switch\n\n`;
  cli += `configure terminal\n!\n`;

  // 1. Global L2 & STP Settings
  cli += `! === SPANNING TREE PROTOCOL & L2 GLOBALS ===\n`;
  if (stp && stp.mode !== 'none') {
    ...
    cli += `spanning-tree mode ${modeMap[stp.mode] || 'rapid-pvst'}\n`;
  } else {
  ...
  if (globalL2) {
    ...
    if (globalL2.errdisableRecovery) {
      ...
      cli += `errdisable recovery interval ${globalL2.errdisableInterval || 300}\n`;
    }
  }
  cli += `!\n`;

  // 2. Security (DHCP Snooping / DAI)
  cli += `! === GLOBAL SECURITY & FABRIC POLICIES ===\n`;
  if (security) {
  ...
  // 3. VLANs
  cli += `! === VLAN DATABASE & TOPOLOGY ===\n`;
  vlans.forEach(vlan => {
    if (vlan.vlanId) {
      cli += `vlan ${vlan.vlanId}\n`;
      if (vlan.name) cli += `   name ${vlan.name}\n`;
      if (vlan.description) cli += `   ! Description: ${vlan.description}\n`;
      if (vlan.stpRoot === 'primary') cli += `spanning-tree vlan ${vlan.vlanId} priority 4096\n`;
      else if (vlan.stpRoot === 'secondary') cli += `spanning-tree vlan ${vlan.vlanId} priority 16384\n`;
    }
  });
  cli += `!\n`;
  ...
  // Generate Physical Interfaces (Arista uses Ethernet1, Ethernet2, etc.)
  uniqueInterfaces.forEach(intf => {
    const interfaceName = normalizeInterfaceName(intf.name, intf.number, 'EOS');
    ...
    if (intf.mode === 'trunk') {
      ...
      if (intf.portSecurity) {
        ...
        if (intf.portSecViolation) {
          const violMap = { 'restrict': 'restrict', 'protect': 'protect', 'shutdown': 'shutdown' };
          cli += `   switchport port-security violation ${violMap[intf.portSecViolation] || 'restrict'}\n`;
        }
      }
    }
    cli += `!\n`;
