...
export const buildNxOsConfig = (state) => {
  ...
  let cli = `! Generated via NetConfig Pro NOC Edition Compiler (NX-OS)\n`;
  cli += `configure terminal\n!\n`;

  // Global Features
  cli += `! ENABLE FEATURES\n`;
  if (globalL2.vtpMode && globalL2.vtpMode !== 'transparent') cli += `feature vtp\n`;
  if (interfaces.some(i => i.channelGroup)) cli += `feature lacp\n`;
  if (security.dhcpSnooping) cli += `feature dhcp\n`;
  if (interfaces.some(i => i.udld !== 'disable')) cli += `feature udld\n`;
  cli += `!\n`;
  ...
  const generateIntfCommands = (intf) => {
    ...
    if (intf.portfast) out += ` spanning-tree port type edge\n`;
    ...
    if (intf.udld === 'aggressive') out += ` udld aggressive\n`;
    else if (intf.udld === 'disable') out += ` udld disable\n`;
  ...
  uniqueInterfaces.forEach(intf => {
    ...
    cli += generateIntfCommands(intf);
    if (intf.channelGroup && intf.channelMode) cli += ` channel-group ${intf.channelGroup} mode ${intf.channelMode === 'desirable' ? 'active' : intf.channelMode}\n`;
    if (intf.status === 'disabled') cli += ` shutdown\n`;
    else cli += ` no shutdown\n`;
    cli += `!\n`;
  });
