...
export function Entropy({ className = "", size = 400 }) {
    ...
    function updateNeighbors() {
      particles.forEach(particle => {
        particle.neighbors = particles.filter(other => {
          ...
          const distance = Math.hypot(particle.x - other.x, this.y - other.y); // wait, bug in original?
          // I'll keep the original logic but fix the reference
        });
      });
    }

    // A better updateNeighbors to fix 'this.y' reference in filter callback
    function updateNeighborsFixed() {
      particles.forEach(particle => {
        particle.neighbors = particles.filter(other => {
          if (other === particle) return false;
  ...
  return (
    <div className={`relative bg-black overflow-hidden ${className}`} style={{ width: size, height: size }}>
      <canvas
        ref={canvasRef}
